...
const reminderRefreshLocks = new Map()
const REMINDER_REFRESH_TTL_MS = Number(process.env.SETTLEMENT_REMINDER_REFRESH_TTL_MS || 60000)

function getReminderRefreshKey(userId) {
  return normalizeId(userId)
}

async function withReminderRefreshLock(userId, work) {
  const key = getReminderRefreshKey(userId)
  const now = Date.now()
  const existing = reminderRefreshLocks.get(key)
  ...
  const promise = Promise.resolve()
    .then(work)
    .finally(() => {
      const current = reminderRefreshLocks.get(key)
      if (current?.promise === promise) {
        reminderRefreshLocks.delete(key)
      }
    })

  reminderRefreshLocks.set(key, {
    expiresAt: now + REMINDER_REFRESH_TTL_MS,
    promise,
  })
...
class NotificationService {
  ...
  static async getUserNotifications(userId, { page = 1, limit = 20, unreadOnly = false } = {}) {
    ...
    const [notifications, total, unreadCount] = await Promise.all([
      ...
      Notification.countDocuments({ userId: normalizeId(userId), read: false }),
    ])
  ...
  static async createSettlementRemindersForUser(userId, options = {}) {
    return withReminderRefreshLock(userId, async () => {
      ...
      const [groups, existingRecent] = await Promise.all([
      ...
      const docs = overdueByGroup
        .filter((row) => !existingGroupIds.has(normalizeId(row._id)))
        .map((row) => {
